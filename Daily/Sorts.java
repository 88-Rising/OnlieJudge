...
public class Sorts {
    ...
    private static void heapify(int[] array, int length, int index) {//堆化
        int left = 2 * index + 1;
        while (left < length) {
            int max = left;
            ...
            swap(array, index, max);//将两个数交换，但是数组下边还没变
            index = max;//改变数组下标，让父节点指针指向孩子结点
            left = 2 * index + 1;
        }
    }
